...
void loop(){
  ...
  if (TIMING_DEBUG) {
    ...
    Serial.println(digitalRead(joystickButtonPin));
    Serial.print("joystick button counter = ");
...
void ForearmMovement(){ //moves the forarm 90 when button pressed
  if(digitalRead(joystickButtonPin) && millis()>=time+200){
